...
async def fetch_new_funding_history(symbol: str, start_ms: int, end_ms: int, limit=1000):
    ...
    while since <= end_ms:
        try:
            partial = await mexc.fetch_funding_rate_history(symbol, since=since, limit=limit)
            if not partial:
                break
...
async def process_symbol(symbol: str, timestamps: dict, now: datetime, results: dict,
    ...
    async with semaphore:
        try:
            ...
            next_time = None
            try:
                fr = await mexc.fetch_funding_rate(symbol)
            ...
            open_interest = None
            try:
                oi_data = await mexc.fetch_open_interest(symbol)
...
async def main():
    ...
    with open(DATA_DIR / "tradePairsMexc.json", "r", encoding="utf-8") as f:
        symbols = json.load(f)
    ...
    out_file = DATA_DIR / "funding_results_mexc.json"
