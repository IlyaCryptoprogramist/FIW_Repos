...
GLOBAL_RATE_LIMIT_MS = hyper.rateLimit * 2
...
async def analyze_trades_activity(symbol: str, hours_back: int = 1):
    """
    Для Hyperliquid публичные сделки недоступны без авторизации,
    поэтому всегда возвращаем нули – фильтры по сделкам будут пропущены.
    """
    return {
        'total_volume_usd': 0,
        'trade_count': 0,
...
async def fetch_ticker_info(symbol: str):
    try:
        ...
        volume_24h_usd = ticker.get('quoteVolume') or info.get('dayNtlVlm', 0)
...
async def fetch_full_funding_history(symbol: str, start_time_ms: int, end_time_ms: int, limit: int = 200):
    ...
    while iteration_count < max_iterations:
        ...
        try:
            partial_history = await hyper.fetch_funding_rate_history(
                symbol=symbol,
                since=current_since,
                limit=limit
            )
...
async def process_symbol(symbol: str, timestamps: dict, now: datetime, results: dict,
    ...
    async with semaphore:
        try:
            ...
            # Анализ сделок – всегда возвращает нули
            trades_info = await analyze_trades_activity(symbol, hours_back=1)

            filters_passed = {
                'min_volume_24h': volume_24h_usd >= min_volume_24h_usd,
                'min_trade_count': True,  # всегда пропускаем
                'recent_trades': True,    # всегда пропускаем
                'min_avg_trade_size': True  # всегда пропускаем
            }
            ...
            print(f"❌ Ошибка при обработке {symbol} на Hyperliquid: {e}")
...
async def main():
    ...
    # ========== НАСТРОЙКИ ФИЛЬТРАЦИИ (только объём 24ч) ==========
    FILTERS = {
        'min_volume_24h_usd': 500000,   # Объём > 500k$
        'min_trade_count_per_hour': 0,  # отключены
    ...
    try:
        ...
        print(f"\n{'='*80}")
        print(f"✅ Результаты Hyperliquid сохранены в: {output_file}")
