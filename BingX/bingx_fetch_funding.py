...
async def analyze_trades_activity(symbol: str, hours_back: int = 1):
    try:
        ...
        total = sum(t.get('cost', 0) for t in trades)
        cnt = len(trades)
        avg = total / cnt
...
async def fetch_new_funding_history(symbol: str, start_ms: int, end_ms: int, limit=1000):
    ...
    while since <= end_ms:
        try:
            partial = await bingx.fetch_funding_rate_history(symbol, since=since, limit=limit)
            if not partial:
                break
...
async def process_symbol(symbol: str, timestamps: dict, now: datetime, results: dict,
                         tickers: dict, min_vol: float, min_trades: int,
                         max_last_sec: int, min_avg: float):
    async with semaphore:
        try:
            ...
            next_time = None
            try:
                fr = await bingx.fetch_funding_rate(symbol)
                current_fr = fr.get('fundingRate')
            ...
            open_interest = None
            try:
                oi_data = await bingx.fetch_open_interest(symbol)
...
async def main():
    ...
    with open(DATA_DIR / "tradePairsBingX.json", "r", encoding="utf-8") as f:
        symbols = json.load(f)

    FILTERS = {'min_vol': 500000, 'min_trades': 100, 'max_last_sec': 25, 'min_avg': 10}
    print("📡 Получаем тикеры...")
    ...
    out_file = DATA_DIR / "funding_results_bingx.json"
